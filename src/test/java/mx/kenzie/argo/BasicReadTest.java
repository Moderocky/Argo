package mx.kenzie.argo;

import mx.kenzie.grammar.Container;
import mx.kenzie.grammar.Null;
import mx.kenzie.grammar.Series;
import org.junit.Test;

import java.util.List;
import java.util.Map;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertNotNull;

public class BasicReadTest {

    @Test
    public void simple() {
        Json.Read json = Json.simple().reader("{}");
        final Container map = json.readContainer();
        assertNotNull(map);
        assertEquals(0, map.size());
    }

    @Test
    public void spaced() {
        Json.Read json = Json.simple().reader("""
                {
                }
                """);
        final Container map = json.readContainer();
        assertNotNull(map);
        assertEquals(0, map.size());

    }

    @Test
    public void keyValue() {
        final String string = """
                { "hello": "there" }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 1;
        assert map.get("hello").equals("there");
    }

    @Test
    public void multiKey() {
        final String string = """
                { "hello": "there", "general": "kenobi" }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 2;
        assert map.get("hello").equals("there");
        assert map.get("general").equals("kenobi");
    }

    @Test
    public void replaceKey() {
        final String string = """
                { "hello": "world", "hello": "there" }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 1;
        assert !map.get("hello").equals("world");
        assert map.get("hello").equals("there");
    }

    @Test
    public void booleans() {
        final String string = """
                { "a": true, "b": false }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 2;
        assert map.get("a").equals(true);
        assert map.get("b").equals(false);
    }

    @Test
    public void numbers() {
        final String string = """
                { "a": 1, "b": 6, "c": 23, "d": -4.5 }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 4;
        assert map.get("a").equals(1) : map;
        assert map.get("b").equals(6) : map;
        assert map.get("c").equals(23) : map;
        assert map.get("d").equals(-4.5) : map;
    }

    @Test
    public void noValue() {
        final String string = """
                { "hello": true, "there": null }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assertNotNull(map);
        assertEquals(2, map.size());
        assertEquals(true, map.get("hello"));
        assertEquals(Null.INSTANCE, map.get("there"));
    }

    @Test
    public void mixed() {
        final String string = """
                { "a": "yes", "b": true, "c": 1, "d": null }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assertNotNull(map);
        assertEquals(4, map.size());
        assertEquals("yes", map.get("a"));
        assertEquals(true, map.get("b"));
        assertEquals(1, (int) map.<Integer>get("c"));
        assertEquals(Null.INSTANCE, map.get("d"));
    }

    @Test
    public void compound() {
        final String string = """
                { "hello": { "hello": "there" } }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 1;
        assert map.get("hello") instanceof Map<?, ?>;
        assert ((Map<?, ?>) map.get("hello")).get("hello").equals("there");
    }

    @Test
    public void emptyList() {
        final String string = """
                { "hello": [ ] }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 1;
        assert map.get("hello") instanceof List<?>;
        assert map.get("hello") != null;
        assert ((List<?>) map.get("hello")).isEmpty();
    }

    @Test
    public void singleList() {
        final String string = """
                { "hello": [ "there" ] }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 1;
        assert map.get("hello") instanceof List<?>;
        assert map.get("hello") != null;
        assert ((List<?>) map.get("hello")).size() == 1;
        assert ((List<?>) map.get("hello")).contains("there");
    }

    @Test
    public void multiList() {
        final String string = """
                { "hello": [ 1, 2, 6.5 ] }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 1;
        assert map.get("hello") instanceof List<?>;
        assert map.get("hello") != null;
        assert ((List<?>) map.get("hello")).size() == 3;
        assert ((List<?>) map.get("hello")).contains(6.5);
        assert ((List<?>) map.get("hello")).contains(1);
    }

    @Test
    public void complexList() {
        final String string = """
                { "hello": [ "there", [ {}, 2 ] ] }
                """;
        Json.Read json = Json.simple().reader(string);
        final Container map = json.readContainer();
        assert map != null;
        assert map.size() == 1;
        assert map.get("hello") instanceof List<?>;
        assert map.get("hello") != null;
        final List<?> list = map.get("hello");
        assert list.size() == 2;
        assert list.contains("there");
        assert list.get(1) instanceof List<?> inner && inner.size() == 2;
        assert list.get(1) instanceof List<?> inner && inner.contains(2);
    }

    @Test
    public void onlyList() {
        final String string = """
                ["hello", "there"]
                """;
        Json.Read json = Json.simple().reader(string);
        final Series list = json.readSeries();
        assertNotNull(list);
        assertEquals(2, list.size());
        assertEquals("hello", list.get(0));
        assertEquals("there", list.get(1));
    }

    @Test
    public void escapes() {
        class Result {
            String hello;
        }
        final String string = "{\"hello\": \"there\\ngeneral\\tkenobi\"}";
        final Result result = Json.fromString(string, Result.class);

        assert result != null;
        assert result.hello != null;
        assert result.hello.equals("there\ngeneral\tkenobi") : result.hello;
    }

}
